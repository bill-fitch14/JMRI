...
class Inglenook():
    ...
    def moveTruckToDesiredPosition(self, truckNo, destSiding):
        ...
        if self.truckInSiding(truckNo, destSiding):
            ...
            if not self.truckAtPositionFromEnd(truckNo, destSiding, desiredPosition + 1 -1):
                ...
                yield ["display_message", msg]
                msg = "mtdph" + str(truckNo) + " a " +  "truck is in position " + str(mypos)
                yield ["display_message", msg]
                ...
                if (self.truckAtPositionFromEnd(truckNo, destSiding, refPos) or
                    ...
                    if noTrucks > 0:
                        ...
                        msg = "mtdpi" + str(truckNo) + " " +  "move " + str(noTrucks) + " from " + str(destSiding) + " to " + str(freeBranch) + " as truck is in position " + str(mypos)
                        yield ["display_message", msg]

                        for p in self.moveTrucksCreatingYieldStatements(noTrucks, destSiding, freeBranch): yield p
                        msg = "mtdpj" + str(truckNo) + " " +  "b1 refPos " +str(refPos+2)
                        yield ["display_message", msg]
        ...
        if (not self.truckInSiding(truckNo, destSiding)):
            ...
            if freeBranch == destSiding:
                ...
                self.moveTruckToDesiredPosition(self, truckNo, destSiding)   #start again with truck in destSiding
            else:
    ...
    def insertTruck(self, from_branch, destination_branch, refPos) :      # counts from 1

        msg = "ita"+ " " +  "insertTruck: " + "from_branch " + str(from_branch) + " destination_branch " + str(destination_branch) + " refPos " + str(refPos)
        yield ["display_message", msg]
        ...
        msg = "ite"+ " " +  " now have only max 2 trucks on destination_branch : "  + str(destination_branch)
        yield ["display_message", msg]
        ...
        if no_trucks_to_move2 > 0:
            msg = "ith"+ " " +  "move2: move " +str(no_trucks_to_move2) + " trucks from head to toBranch"
            yield ["display_message", msg]
            freeBranch = 0
            ...
            for p in self.moveTrucksCreatingYieldStatements(no_trucks_to_move2, 4, freeBranch): yield p
    ...
    def moveTrucksCreatingYieldStatements(self, noTrucks, fromBranch, destBranch):
        ...
        if fromBranch < self.spur_branch or destBranch == self.spur_branch or destBranch == mid_branch:   #if not at spur, move to spur
            for i in range(0, noTrucks):
                # msg = "mtcys"+ " " +  "move Truck to spur: " + str(i)
                # yield ["display_message", msg]
        ...
        msg = "mtcys"+ " " +  "finished moveTrucksCreatingYieldStatements " + str(self.pegs)
        yield ["display_message", msg]
    ...
    def getFreeBranch(self, noTrucksRequired, currentBranch):
        ...
        if self.noSpacesinBranch(otherBranch1) <= noTrucksRequired or self.noSpacesinBranch(otherBranch2) <= noTrucksRequired:
            ...
            # move the trucks between the brtanches so there is enough room
